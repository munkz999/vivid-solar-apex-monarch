...
export function CardTab() {
  ...
  const rows = buildChart({
    ...
    effort: 100,
    conditions,
    benchmarks: preset === "fit" ? benchmarks : [],
    lockSpeed: preset === "fit",
  });
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="font-display text-xl font-medium tracking-tight">Bag card</h2>
        <p className="text-sm text-muted">
          4×6 print or camera-roll image. Same numbers as Chart — take it to the course.
        </p>
      </div>
      ...
      {yoursCount === 0 ? (
        <p className="no-print px-1 text-center text-xs text-faint">
          Log shots in Fit to stamp YOURS on the card.
        </p>
      ) : null}

      <button
        type="button"
        className="no-print text-center text-sm text-muted"
